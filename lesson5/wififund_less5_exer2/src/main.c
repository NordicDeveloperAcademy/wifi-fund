...
static int server_connect(void)
{
	...
	if (sock < 0) {
		LOG_INF("Failed to set up HTTPS socket, err: %d, %s", errno, strerror(errno));
		return -errno;
	}
...
int main(void)
{
	...
	if (dk_leds_init() != 0) {
		LOG_INF("Failed to initialize the LED library");
	}
	
	if (wifi_connect() != 0) {
		LOG_INF("Failed to connect to Wi-Fi");
	}

	if (dk_buttons_init(button_handler) != 0) {
		LOG_INF("Failed to initialize the buttons library");
	}
	if (server_resolve() != 0) {
